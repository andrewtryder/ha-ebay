"""Section fetch orchestration for EbayApiClient.

Moves the coordinator payload assembly loop out of ``api.py`` while keeping
per-API HTTP on client mixins under ``clients/``.
"""
...
from .clients.scopes import missing_scopes_for_options
from .const import (
...
from .parsers.payload import summarize_payload
...
def _seller_ops_summary_keys(seller_ops: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "listing_amount_used": privileges.get("amount_used"),
        "listing_amount_currency": privileges.get("amount_currency"),
        "listing_quantity_remaining": privileges.get("quantity_remaining"),
        "listing_quantity_used": privileges.get("quantity_used"),
...
class SectionFetchMixin:
    """Assemble coordinator payloads by fetching due sections."""
